...
function Controller() {
    ...
    _.extend($, $.__views);
    $.tabList.title = L("list", "Comprobacion");
    $.list.title = L("list", "Comprobacion");
    ...
    $.list.addEventListener("focus", function() {
        if ($.list.activity) {
            ...
            Alloy.Globals.Android.Api >= 11 && activity.actionBar && (activity.actionBar.title = L("list", "Lista Solicitud de Viajes"));
        }
    });
    _.extend($, exports);
}
